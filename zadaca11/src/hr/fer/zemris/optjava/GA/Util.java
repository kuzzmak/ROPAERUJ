	...
	public static void mutate(GASolution<int[]> solution, IRNG rng, double p) {
		...
		for (int i = 1; i < solution.data.length - 1; i += 5) {
			...
			if (p > rng.nextDouble()) {
				...
				if ((solution.data[i + 1] + rand * randHeight <= height)
						&& (solution.data[i + 1] + rand * randHeight >= 0)) {
				...
				if (!(solution.data[i + 4] + rand * randColor > 127
						|| solution.data[i + 4] + rand * randColor < -128)) {
...
//	}
	...
	public static GASolution<int[]> select(List<GASolution<int[]>> population, IRNG rng) {
		...
		// oduzimanje najmanjeg fitnesa od svih ostalih kako bi se dobili pozitivni
		// fitnesi
	...
	public static void write (String filename, GASolution<int[]> solution) throws IOException{
