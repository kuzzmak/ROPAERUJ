...
public class PSO {
	...
	public PSO(int populationSize, NeuralNet net, int maxIterations, double minError, char version) {

		PSO.populationSize = populationSize;
		this.net = net;
		PSO.particleSize = net.getNumOfWeights();
	...
	public PSO(int populationSize, NeuralNet net, int maxIterations, double minError, char version, int numOfNeighbours) {
	...
	public double[] run() {
		...
		while (iteration < maxIterations && error > minError) {
			...
			for (int i = 0; i < populationSize; i++) {
				...
				case 'a':
					for (int j = 0; j < particleSize; j++) {
						...
						if (currentSpeed[j] > vmax)
							currentSpeed[j] = vmax;
						if (currentSpeed[j] < vmin)
							currentSpeed[j] = vmin;
				...
				case 'b':
					for (int j = 0; j < particleSize; j++) {
						...
						currentSpeed[j] = inertiaFac * currentSpeed[j] + 
								c1 * rnd * (p.getPersonalBest()[j] - currentLocation[j]) + 
								c2 * rnd * (pickNeighbour(p).getPersonalBest()[j] - currentLocation[j]);
			...
			System.out.println(
					"iteration: " + iteration + ", value: " + globalBest.getBestValue() + ", inertia: " + inertiaFac);
		}
	...
	public Particle pickNeighbour(Particle p) {
		...
		Particle[] nearest = new Particle[numOfNeighbours * 2];
		// pronalazak najblizih po indeksu
		for (int i = 0; i < numOfNeighbours * 2; i++) {
		...
		for (int i = 1; i < nearest.length; i++) {
			if (nearest[i].getBestValue() > bestNeighbour.getBestValue())
				bestNeighbour = nearest[i];
		}
